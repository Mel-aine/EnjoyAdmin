<template>
        ...
        <div class="card live-feed-card">
          ...
          <div class="live-feed-content">
            <div class="event-item" v-for="event in filteredEvents" :key="event.id">
              <div class="event-time">{{ getTimeAgoLabel(event.timeAgo) }}</div>
              <div class="event-badge" :class="event.type.toLowerCase().replace(' ', '-')">{{ getEventTypeLabel(event.type) }}</div>
 ...
 const handleMigrate = async () => {
  ...
  const filterOptions = computed(() => [
    { value: 'all', label: t('configuration.channelManager.allEvents') },
    { value: 'bookings', label: t('configuration.channelManager.newBookings') },
    { value: 'cancellations', label: t('configuration.channelManager.cancellations') }
  ])
  ...
  const getEventTypeLabel = (type: string) => {
    const typeMap: Record<string, string> = {
      'NEW BOOKING': 'configuration.channelManager.eventTypes.newBooking',
      'CANCELLATION': 'configuration.channelManager.eventTypes.cancellation'
    }
    return t(typeMap[type] || type)
  }
 
  const getTimeAgoLabel = (timeAgo: string) => {
    if (timeAgo === 'a day ago') {
      return t('configuration.channelManager.timeAgo.aDayAgo')
    }
    return timeAgo
  }
