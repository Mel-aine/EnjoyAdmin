    ...
    <AdminLayout>
      <div class="min-h-screen">
        ...
        <div class="flex flex-col gap-10 mt-20 ">
          <TableOne :items="titles" :datas="customers" :options="filterOptions" :filterable="true" :pagination="true"
            @view="onViewUser" />
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  ...
                  <div>
                    ...
                    <div class="relative">
                      ...
                      <input v-model="phoneNumber" placeholder="+237 693774450" type="tel"
        ...
        <div
          ...
          <div v-if="selectedCustomer" class="space-y-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
              ...
              <div><span class="font-medium">{{ $t('ArrivedDate') }} :</span> {{
                formatDate(selectedCustomer.arrivedDate) }}</div>
              <div><span class="font-medium">{{ $t('DepartDate') }} :</span> {{ formatDate(selectedCustomer.departDate)
                }}</div>
              <div><span class="font-medium">{{ $t('reservationType') }} :</span> {{ selectedCustomer.reservationType }}
              </div>
              <div><span class="font-medium">{{ $t('personNumber') }} :</span> {{ selectedCustomer.totalPerson }}</div>
              <div><span class="font-medium">{{ $t('TotalPrice') }} :</span> {{ selectedCustomer.totalPrice }} FCFA
              </div>
              <div><span class="font-medium">{{ $t('Payment') }} :</span> <span
                  :class="statusClass1(selectedCustomer.payment)"> {{ selectedCustomer.payment }}</span></div>
              ...
              <div><span class="font-medium">{{ $t('Createdon') }} :</span> {{
                formatDateTime(selectedCustomer.createdAt) }}</div>
              <div><span class="font-medium">{{ $t('Modifiedon') }}:</span> {{
                formatDateTime(selectedCustomer.updatedAt) }}</div>
            </div>
          </div>
...
const onViewUser = (c: any) => {
  selectedCustomer.value = c;
  showModal.value = true;
  console.log('Client sélectionné :', selectedCustomer.value);
};
...
const fetchCustomers = async () => {
  try {
    ...
    customers.value = response.data.data.map((user: any) => {
      return {
        ...user,
...
const titles = computed(() => [
  ...
  },
  {
    ...
    render: (value: string) => {
      return value === 'active'
        ? `<span class="bg-success-50 text-success-700 px-2 rounded-full dark:bg-success-500/15 dark:text-success-500">Active</span>`
        : `<span class="bg-red-50 text-red-700 px-2 rounded-full dark:bg-red-500/15 dark:text-red-500">Inactive</span>`;
    }
  },
  {
    name: 'actions',
    ...
    actions: [
      {
        ...
        color: 'bg-blue-100 text-blue-600 px-2 py-1 rounded-full'
      }
    ],
    colored: true
  }
