                ...
                <div class="p-6" v-if="user && user.id">
                    ...
                    <Modal v-if="isCreateContractModalOpen" @close="isCreateContractModalOpen = false">
                        <template #body>
                            <div
                                ...
                                <h2 class="text-lg font-semibold mb-4">{{ contractFormAction == "CREATE" ? $t('new') :
                                    $t('edit') }}</h2>
                                <form @submit.prevent="handleAssignContract">
                                    <div class="mb-4">
                                        <InputDatePicker v-model="contractInfoFormData.probation_start_date"
                                            :is-required="true"
                                            @clear-error="contractInfoFormData.probation_start_date_error = ''"
                                            :title="$t('probation_start_date')" />
                                        <div v-if="contractInfoFormData.probation_start_date_error"
                                    ...
                                    <div class="mb-4">
                                        <InputDatePicker v-model="contractInfoFormData.probation_end_date"
                                            :is-required="true"
                                            @clear-error="contractInfoFormData.probation_end_date_error = ''"
                                            :title="$t('probation_end_date')" />
                                        <div v-if="contractInfoFormData.probation_end_date_error"
                                            class="text-red-500 text-sm italic">
                                    ...
                                    <div class="mb-4">
                                        <InputDatePicker v-model="contractInfoFormData.contract_start_date"
                                            :is-required="true"
                                            @clear-error="contractInfoFormData.contract_start_date_error = ''"
                                            :title="$t('contract_start_date')" />
                                        <div v-if="contractInfoFormData.contract_start_date_error"
                                            class="text-red-500 text-sm italic">
                                    ...
                                    <div class="mb-4">
                                        <label for="nonRefundableRateEnabled" class="flex items-center cursor-pointer">
                                            <span class="text-gray-700 font-medium">{{ $t('is_cdi')
                                                }}</span>&nbsp;&nbsp;&nbsp;
                                            <input type="checkbox" id="nonRefundableRateEnabled"
                                                name="nonRefundableRateEnabled" v-model="contractInfoFormData.is_cdi"
                                                class="form-checkbox h-5 w-5 text-purple-600 rounded" />
                                        </label>
                                    </div>

                                    <div class="mb-4" v-if="!contractInfoFormData.is_cdi">
                                        <InputDatePicker v-model="contractInfoFormData.contract_end_date_error"
                                            :is-required="true"
                                            @clear-error="contractInfoFormData.contract_end_date_error = ''"
                                            :title="$t('contract_end_date')" />
                                        <div v-if="contractInfoFormData.contract_end_date_error"
                                            class="text-red-500 text-sm italic">
                                    ...
                                    <div class="mb-4">
                                        <div>
                                            ...
                                            <div v-if="contractInfoFormData.contract_file_path_error"
                    ...
                    <Modal v-if="isAddPayrollModalOpen" @close="closeAddPayrollModal">
                        <template #body>
                            <div
                                class="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
                                <button @click="closeAddPayrollModal"
                                ...
                                <form @submit.prevent="handleCreatePayroll">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <!-- Mois / Année -->
                                        <div>
                                            <InputDatePicker v-model="payrollForm.monthYear" :is-required="true"
                                                :title="$t('monthYear')"
                                        ...
                                        <!-- Salaire brut -->
                                        <div>
                                            <InputCurrency v-model="payrollForm.grossSalary" :lb="$t('grossSalary')"
                                                :placeholder="$t('grossSalary')" />
                                            <div v-if="payrollForm.grossSalary_error"
                                                class="text-red-500 text-sm italic">
                                        ...
                                        <!-- Heures normales -->
                                        <div>
                                            <Input inputType="number" v-model.number="payrollForm.normalHours"
                                                :lb="$t('normalHours')" :isRequired="true"
                                                @clear-error="payrollForm.normalHours_error = ''" />
                                            <div v-if="payrollForm.normalHours_error"
                                                class="text-red-500 text-sm italic">
                                        ...
                                        <div>
                                            <Input inputType="number" v-model.number="payrollForm.overtimeHours"
                                                :lb="$t('overtimeHours')"
                                                @clear-error="payrollForm.overtimeHours_error = ''" />
                                            <div v-if="payrollForm.overtimeHours_error"
                                                class="text-red-500 text-sm italic">
                                        ...
                                        <div>
                                            <InputCurrency v-model="payrollForm.overtimePay" :lb="$t('overtimePay')" />
                                            <div v-if="payrollForm.overtimePay_error"
                                                class="text-red-500 text-sm italic">
                                        ...
                                        <!-- Primes -->
                                        <div>
                                            <InputCurrency v-model="payrollForm.bonuses" :lb="$t('bonuses')" />
                                        ...
                                        <!-- Indemnités -->
                                        <div>
                                            <InputCurrency v-model="payrollForm.allowances" :lb="$t('allowances')" />
                                            <div v-if="payrollForm.allowances_error"
                                                class="text-red-500 text-sm italic">
                                        ...
                                        <!-- CNPS -->
                                        <div>
                                            <InputCurrency v-model="payrollForm.cnpsContributions"
                                                :lb="$t('cnpsContributions')" />
                                            <div v-if="payrollForm.cnpsContributions_error"
                                                class="text-red-500 text-sm italic">
                                        ...
                                        <!-- Impôts -->
                                        <div>
                                            <InputCurrency v-model="payrollForm.withheldTaxes"
                                                :lb="$t('withheldTaxes')" />
                                            <div v-if="payrollForm.withheldTaxes_error"
                                                class="text-red-500 text-sm italic">
                                        ...
                                        <!-- Salaire net -->
                                        <div>
                                            <InputCurrency v-model="payrollForm.netSalary" :disabled="true"
                                                :lb="$t('netSalary')" />
                                    ...
                                    <div class="flex justify-end space-x-3 mt-6">
                                        <button type="button" @click="isAddPayrollModalOpen = false"
                                            class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">
                                            {{ $t('Cancel') }}
                                        </button>
                                        ...
                                        <div class="">
                                            ...
                                            <div class="text-center mb-15">
                                                <p class="text-gray-600 text-sm leading-relaxed">{{
                                                    $t('contractAssignmentDescription') }}
                                                </p>
                                            </div>
                                        ...
                                        <div class="w-full">
                                            <div class="flex justify-between items-start sm:items-center mb-2 lg:mb-6">
                                                ...
                                                <div class="flex justify-end gap-3">
                                                    <button type="button" @click="editContract"
                                                    ...
                                                    <button type="button" @click="showTerminationModal = true"
                                            ...
                                            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-20">
                                                ...
                                                <div>
                                                    ...
                                                    <p
                                                        :class="`${getStatusColor(user.recentContract.status)} rounded-2xl text-sm py-1 px-2 w-fit`">
                                                        {{
...
-->
                            ...
                            <div v-if="activeTab === 'payroll'">
                                <TableComponent :items="payrollTitles" :datas="payrollsForTable"
                                    :loading="payrollLoading" :title="$t('payrolls')" :pagination="true"
                                    @downloadPayslip="downloadPayslip" :showButtonAllElement="true">
