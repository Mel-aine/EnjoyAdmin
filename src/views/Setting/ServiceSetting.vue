<template>
  <div class="max-w-full mx-auto p-6">
    ...
    <div class="max-w-full mx-auto p-6 bg-white rounded-lg  py-8 mb-8">
      ...
      <div class="mb-8">
        ...
        <div
          ...
          <input type="file" accept="image/*" @change="handleLogoFile" class="hidden" ref="logoInput"
            id="logo-upload" />
          ...
          <div v-if="logoPreview" class="relative">
            <img :src="logoPreview" alt="Logo preview" class="mx-auto max-h-32 object-contain rounded-lg" />
            <div class="mt-3 flex justify-center space-x-2">
              <button @click="logoInput?.click()"
                class="px-3 py-1 bg-purple-500 text-white text-sm rounded hover:bg-purple-600">
                {{ $t('change') }}
              </button>
              <button @click="removeLogo" class="px-3 py-1 bg-red-500 text-white text-sm rounded hover:bg-red-600">
                {{ $t('delete') }}
              </button>
            </div>
          </div>
      ...
      <div class="mb-8">
        ...
        <div
          class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-purple-400 transition-colors">
          <input type="file" accept="image/*" multiple @change="handleFiles" class="hidden" ref="fileInput"
            id="images-upload" />
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            <div v-for="(preview, index) in imagePreviews" :key="index" class="relative group">
              ...
              <div
                ...
                <button @click="removeImage(index)" class="p-2 bg-red-500 text-white rounded-full hover:bg-red-600">
      ...
      <div v-if="uploadedUrls.length > 0" class="mt-8 p-4 bg-green-50 rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="(result, index) in uploadedUrls" :key="index"
    ...
    <!--section heure de checkin et checkout-->
    <div class="mb-8 max-w-full mx-auto p-6 bg-white rounded-lg">
      <h3 class="text-xl font-semibold text-gray-700 mb-4">{{ $t('checkinCheckoutSettings') }}</h3>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          <TimePicker :label="t('Arrival from')" v-model="hotelInfo.checkin_hours" />
        </div>

        <div>
          <TimePicker :label="t('Departure before')" v-model="hotelInfo.checkout_hours" />
        </div>
      </div>
    </div>
    ...
    <div class="mb-8 max-w-full mx-auto p-6 bg-white rounded-lg">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
          <Input :lb="t('taxes_and_fees.vat_hospitality.name')" :inputType="'number'" :id="'taxRate'" :forLabel="'taxRate'"
            v-model.number="pricingParameters.vat_hospitality" step="0.01" min="0" />
            <label class="text-sm text-gray-700">{{ t('taxes_and_fees.vat_hospitality.description') }}</label>
        </div>
        <div>
          <Input :lb="t('taxes_and_fees.tourist_tax.name')" :inputType="'number'" :id="'taxRate'" :forLabel="'taxRate'"
            v-model.number="pricingParameters.tourist_tax" step="0.01" min="0" :disabled="true" />
            <label class="text-sm text-gray-700">{{ t('taxes_and_fees.tourist_tax.description') }}</label>
        </div>
        <div>
          <Input :lb="t('taxes_and_fees.general_vat.name')" :inputType="'number'" :id="'taxRate'" :forLabel="'taxRate'" 
            v-model.number="pricingParameters.general_vat" step="0.01" min="0" />
            <label class="text-sm text-gray-700">{{ t('taxes_and_fees.general_vat.description') }}</label>
        </div>
      </div>

      <PaymentMethod v-model="selectedPaymentMethods" @update:modelValue="handlePaymentMethodsUpdate" />
    ...
    <div class="flex justify-end gap-4">
      <button type="button" @click="updateParameters"
        ...
        <svg v-if="isLoading" class="animate-spin -ml-1 mr-3 h-4 w-4 text-white inline-flex"
...
import TimePicker from '@/components/forms/FormElements/TimePicker.vue'
...
const hotelInfo = ref({
  ...
  email: '',
  checkin_hours: "00:00",
  checkout_hours: "00:00",
  stars: 0,
})

const pricingParameters = ref<PricingParameters>({
  currency: 'XAF',
  general_vat: 19.25,
  tourist_tax: 0,
  vat_hospitality: 18,
...
onMounted(async () => {
  try {
    ...
    hotelInfo.value = {
      ...
      checkin_hours: data.data.checkinHours ?? '00:00',
      checkout_hours: data.data.checkoutHours ?? '00:00',
      stars: data.data.stars,
    }
    pricingParameters.value={
      currency: data.data.currency,
      general_vat: data.data.generalVat,
      tourist_tax: 1000*data.data.stars,
      vat_hospitality: data.data.vatHospitality,
    }
...
const updateParameters = async () => {
  ...
  try {
    const serviceId = serviceStore.serviceId
    console.log('logo', logoUrl.value)
    console.log('Images', Images.value)
    console.log('selectedPaymentMethods', Logo.value)
    ...
    const payload = {
      ...
      logo: Logo.value ?? logoUrl.value,
      images: JSON.stringify(Images.value),
      checkin_hours: hotelInfo.value.checkin_hours,
      checkout_hours: hotelInfo.value.checkout_hours,
      stars: hotelInfo.value.stars,
      general_vat: pricingParameters.value.general_vat,
      tourist_tax: pricingParameters.value.tourist_tax,
      vat_hospitality: pricingParameters.value.vat_hospitality,
      currency: pricingParameters.value.currency,
    }
