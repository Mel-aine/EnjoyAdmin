        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <div class="flex items-center justify-between mb-4">
            <div>
              <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2">
                <Shield class="w-6 h-6 text-primary/80" />
                {{ $t('manage_permission') }}
              </h1>
          ...
          <div class="flex items-center justify-between mb-4">
            <div class="relative w-2/2">
              ...
              <input type="text" v-model="searchTerm" :placeholder="$t('search_role')"
                class="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:outline-none focus:ring-purple-500 " />
            </div>
              ...
              <div class="space-y-2 max-h-96 overflow-y-auto">
                <button v-for="role in filteredRoles" :key="role.id" @click="selectedRole = role" :class="selectedRole?.id === role.id
                  ...
                  <div class="text-sm text-gray-500 mt-1">
                    ...
                    }}{{ role.permissions?.service?.length > 0 ? 's' : '' }}
                  </div>
                </button>
              </div>
            </div>
            ...
            <div v-if="editMode && selectedRole" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div class="flex justify-between items-center mb-6">
                ...
                <div class="flex justify-end gap-3">
                  <button @click="toggleEditMode" :class="'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100'
                    " class="px-4 py-2 rounded-lg border transition-colors">
                    {{ $t('Cancel') }}
                  </button>
                  <button v-if="editMode" @click="savePermissions"
                    class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/85 transition-colors">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                <label v-for="permission in allPermissions" :key="permission"
                  ...
                  <div class="flex items-center gap-2">
                    <component :is="iconComponent(permissionLabels[permission]?.icon)" class="w-4 h-4" />
                ...
                <div class="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p class="text-gray-600">
                      {{ selectedRole.permissions?.service?.length || 0 }} {{ $t('permission') }}{{
                        selectedRole.permissions?.service?.length > 0 ? 's' : '' }} {{ $t('assigned') }}
                    </p>
