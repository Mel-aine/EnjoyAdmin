...
<template>
  <AdminLayout>
    ...
    <div v-if="dragNotification"
         class="mb-4 p-4 rounded-lg border-l-4"
         :class="dragNotification.type === 'success' ? 'bg-green-50 border-green-400 text-green-700' : 'bg-red-50 border-red-400 text-red-700'">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <Modal v-if="showConfirmModal" @close="cancelDragOperation">
        <template #body>
          <div class="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-6 dark:bg-gray-900">
            ...
            <div class="mb-6 space-y-3 text-sm text-gray-600 dark:text-gray-400">
              <p><strong>{{ $t('Client') }}:</strong> {{ pendingDragInfo?.title }}</p>
              <p><strong>{{ $t('OldDates') }}:</strong> {{ formatDateRange(pendingDragInfo?.oldStart, pendingDragInfo?.oldEnd) }}</p>
              <p><strong>{{ $t('NewDates') }}:</strong> {{ formatDateRange(pendingDragInfo?.newStart, pendingDragInfo?.newEnd) }}</p>
              <p class="text-blue-600"><strong>{{ $t('Duration') }}:</strong> {{ calculateNights(pendingDragInfo?.newStart, pendingDragInfo?.newEnd) }} {{ $t('nights') }}</p>
            </div>

            <div class="flex items-center gap-3 justify-end">
              <button @click="cancelDragOperation"
              ...
              <button @click="confirmDragOperation"
...
const confirmDragOperation = async () => {
  ...
  try {
    // Ici vous devriez appeler votre API pour mettre à jour la réservation
    const updatedReservation = {
      id: pendingDragInfo.value.reservationId,
      arrivedDate: pendingDragInfo.value.newStart.toISOString().split('T')[0],
      departDate: new Date(pendingDragInfo.value.newEnd.getTime() - 24 * 60 * 60 * 1000).toISOString().split('T')[0] // Soustraire 1 jour car FullCalendar ajoute 1 jour à la fin
    };

    // Exemple d'appel API (remplacez par votre vraie fonction)
    ...
    if (success) {
      ...
      const eventIndex = calendarEvents.value.findIndex(e => e.id === pendingDragInfo.value.reservationId);
      if (eventIndex !== -1) {
        calendarEvents.value[eventIndex] = {
          ...calendarEvents.value[eventIndex],
          start: pendingDragInfo.value.newStart.toISOString().split('T')[0],
          end: pendingDragInfo.value.newEnd.toISOString().split('T')[0]
        };
      }

      // Afficher notification de succès
      showNotification(`Réservation mise à jour avec succès. Nouvelles dates: ${formatDateRange(pendingDragInfo.value.newStart, pendingDragInfo.value.newEnd)}`, 'success');
      ...
      showNotification('Erreur lors de la mise à jour de la réservation', 'error');
    }
  } catch (error) {
