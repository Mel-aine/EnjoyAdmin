<template>
  <header
    class="sticky top-0 flex w-full bg-white border-gray-200 z-49 dark:border-gray-800 dark:bg-gray-900 lg:border-b">
    <div class="flex flex-col items-center justify-between grow lg:flex-row lg:px-6 ">
      <div
        ...
        <button @click="handleToggle"
          class="flex items-center justify-center w-10 h-10 text-gray-500 border-gray-200 rounded-lg z-99999 dark:border-gray-800 dark:text-gray-400 lg:h-8 lg:w-8 lg:border"
          :class="[
            isMobileOpen
      ...
      <div :class="[isApplicationMenuOpen ? 'flex' : 'hidden']"
        ...
        <div class="flex items-center gap-2 2xsm:gap-3">
          <button class="flex flex-nowrap px-4" @click="gotoCalendar">
            <Calendar class="w-4 me-2 cursor-pointer" @click="openModal" />
          </button>
          <NotificationMenu />
        </div>
        <UserMenu />
