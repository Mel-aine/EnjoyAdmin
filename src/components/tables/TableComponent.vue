<template>
  <div class="w-full max-w-8xl px-2">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div class="px-4 sm:px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-indigo-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div class="flex items-center gap-3 min-w-0 flex-shrink-0">
            ...
            <div class="min-w-0">
              <h2 class="text-xl font-bold text-gray-900 dark:text-white truncate">{{ title || $t('datas') }}</h2>
          ...
          <!-- Actions et recherche - amélioration responsive -->
      ...
      <!-- Container du tableau avec scroll horizontal approprié -->
      <div class="w-full overflow-x-auto">
        <div class="inline-block min-w-full align-middle">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr>
                ...
                >
                  <div class="flex items-center justify-between group min-w-0">
                    ...
                    >
                      <svg
                        class="w-4 h-4 text-gray-400 hover:text-gray-600 transition-transform duration-200"
                        :class="{
            ...
            <!-- Corps du tableau avec gestion loading -->
            <tbody v-if="props.loading" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="n in 5" :key="n" class="animate-pulse">
                <td v-for="(item, i) in items" :key="i" :class="getOptimizedCellClasses(item, i)" class="px-3 sm:px-4 py-3">
                  <div v-if="item.type === 'image'" class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-300 dark:bg-slate-700"></div>
                  <div v-else-if="item.type === 'badge'" class="h-6 bg-slate-200 dark:bg-slate-600 rounded-full w-16 sm:w-20"></div>
            ...
            <tbody v-else class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr
                ...
                :key="rowIndex"
                class="hover:bg-indigo-50 dark:hover:bg-gray-700 transition-colors duration-150"
              >
                <td
                  ...
                  <div v-if="row[item.name] !== undefined" class="min-w-0 w-full">
                    ...
                    <template v-else-if="item.type === 'imageText' && typeof row[item.name] === 'object'">
                      <div class="flex items-center gap-2 min-w-0 max-w-full">
                        <img :src="row[item.name].image" alt="img" class="w-6 h-6 sm:w-8 sm:h-8 rounded-full object-cover flex-shrink-0" />
                        <span class="truncate text-sm font-medium text-gray-800 dark:text-white min-w-0 flex-1" :title="row[item.name].name">{{ row[item.name].name }}</span>
                      </div>
                    </template>
                      ...
                      <div class="flex items-center gap-1">
                        <template v-for="n in 5" :key="n">
                          <svg
                            ...
                            viewBox="0 0 20 20"
                          >
                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.967a1 1 0 00.95.69h4.176c.969 0 1.371 1.24.588 1.81l-3.385 2.46a1 1 0 00-.364 1.118l1.287 3.966c.3.921-.755 1.688-1.538 1.118l-3.385-2.46a1 1 0 00-1.175 0l-3.385 2.46c-.783.57-1.838-.197-1.538-1.118l1.287-3.966a1 1 0 00-.364-1.118L2.049 9.394c-.783-.57-.38-1.81.588-1.81h4.176a1 1 0 00.95-.69l1.286-3.967z"/>
                          </svg>
                          <svg
                            v-else
                    ...
                    <template v-else-if="item.type === 'TextRoom' && row[item.name] && typeof row[item.name] === 'object'">
                      <div class="flex flex-col min-w-0 max-w-full">
                        <span class="text-sm font-medium text-gray-800 dark:text-white truncate" :title="row[item.name].name || '-'">
                          {{ row[item.name].name || '-' }}
                        </span>
                        <span class="text-xs text-purple-500 dark:text-gray-300 whitespace-nowrap">
                          {{ row[item.name].roomCount ?? 0 }} {{$t('Rooms')}}
                        </span>
                      </div>
                    </template>

                    <!-- Pourcentage optimisé -->
                    <template v-else-if="item.type === 'percentage'">
                      <div class="flex items-center gap-2 min-w-0 max-w-full">
                        <span class="font-medium text-gray-900 dark:text-white whitespace-nowrap text-sm">{{ row[item.name] }}%</span>
                        <div class="w-12 sm:w-16 lg:w-20 bg-gray-200 dark:bg-gray-600 rounded-full h-2 flex-shrink-0">
                          <div
                    ...
                    <template v-else-if="item.type === 'currency'">
                      <span class="text-gray-900 dark:text-white font-medium text-sm whitespace-nowrap">{{ formatCurrency(row[item.name]) }}</span>
                    </template>

                    <!-- Texte par défaut optimisé -->
                    <template v-else>
                      <span class="text-gray-900 dark:text-white font-medium text-sm capitalize truncate block max-w-full" :title="row[item.name]">{{ row[item.name] }}</span>
                    </template>
                  </div>
...
interface TableColumn {
  name: string
  label: string
  type?: 'text' | 'url' | 'badge' | 'percentage' | 'image' | 'action' | 'imageText' | 'rating' | 'currency' | 'TextRoom'
  sortable?: boolean
  filterable?: boolean
  style?: string
  minWidth?: string
...
// Méthodes pour les classes CSS avec gestion des largeurs
const getOptimizedHeaderClasses = (item: TableColumn, index: number) => {
  let classes = ''

  // Largeurs optimisées par type de colonne
  switch (item.type) {
    case 'image':
...
const getOptimizedCellClasses = (item: TableColumn, index: number) => {
  ...
  // Largeurs identiques aux headers
  switch (item.type) {
    case 'image':
      classes += 'w-12 sm:w-16'
      break
