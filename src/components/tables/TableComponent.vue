        ...
        <div class="inline-block min-w-full align-middle">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <thead class="bg-gray-50 dark:bg-gray-700">
              <tr>
                <th v-for="(item, index) in items" :key="index" :class="getOptimizedHeaderClasses(item, index)"
                  ...
                  <div class="flex items-center justify-between group min-w-0">
                    ...
                    <button v-if="item.sortable" @click="toggleSort(item.name)" :class="[
                      ...
                      <svg class="w-4 h-4 text-gray-400 hover:text-gray-600 transition-transform duration-200" :class="{
              ...
              <tr v-for="n in 5" :key="n" class="animate-pulse">
                <td v-for="(item, i) in items" :key="i" :class="getOptimizedCellClasses(item, i)"
                  ...
                  <div v-if="item.type === 'image'"
                    class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-300 dark:bg-slate-700"></div>
            ...
            <tbody v-else class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-for="(row, rowIndex) in paginatedData" :key="rowIndex"
                ...
                <td v-for="(item, colIndex) in items" :key="colIndex" :class="getOptimizedCellClasses(item, colIndex)"
                  ...
                  <div v-if="row[item.name] !== undefined" class="min-w-0 w-full">
                    ...
                    <template v-if="item.type === 'url'">
                      <template v-if="item.inputField">
                        <input :type="item.inputField.type" :placeholder="item.inputField.placeholder"
                    ...
                    <template v-else-if="item.type === 'imageText' && typeof row[item.name] === 'object'">
                      <div class="flex items-center gap-2 min-w-0 max-w-full">
                        ...
                        <span class="truncate text-sm font-medium text-gray-800 dark:text-white min-w-0 flex-1"
                    ...
                    <template v-else-if="item.type === 'rating'">
                      <div class="flex items-center gap-1">
                        <template v-for="n in 5" :key="n">
                          <svg v-if="n <= row[item.name]" class="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 flex-shrink-0"
                          ...
                          <svg v-else class="w-3 h-3 sm:w-4 sm:h-4 text-gray-300 flex-shrink-0" fill="currentColor"
                            viewBox="0 0 20 20">
                            <path
                              d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.967a1 1 0 00.95.69h4.176c.969 0 1.371 1.24.588 1.81l-3.385 2.46a1 1 0 00-.364 1.118l1.287 3.966c.3.921-.755 1.688-1.538 1.118l-3.385-2.46a1 1 0 00-1.175 0l-3.385 2.46c-.783.57-1.838-.197-1.538-1.118l1.287-3.966a1 1 0 00-.364-1.118L2.049 9.394c-.783-.57-.38-1.81.588-1.81h4.176a1 1 0 00.95-.69l1.286-3.967z" />
                          </svg>
                    ...
                    <template v-else-if="item.type === 'percentage'">
                      <div class="flex items-center gap-2 min-w-0 max-w-full">
                        <span class="font-medium text-gray-900 dark:text-white whitespace-nowrap text-sm">{{
                          row[item.name] }}%</span>
                        <div class="w-12 sm:w-16 lg:w-20 bg-gray-200 dark:bg-gray-600 rounded-full h-2 flex-shrink-0">
                          <div class="bg-indigo-600 h-2 rounded-full transition-all duration-300"
                            :style="{ width: `${Math.min(row[item.name], 100)}%` }"></div>
                        </div>
                      </div>
                    </template>
                    ...
                    <template v-else-if="item.type === 'image'">
                      <div class="flex items-center">
                        <img v-if="row[item.name]?.trim()" :src="row[item.name]" :alt="row.title || 'Image'"
                    ...
                    <template v-else-if="item.type === 'currency'">
                      <span class="text-gray-900 dark:text-white font-medium text-sm whitespace-nowrap">{{
...
<span
                  ...
                  <div v-else-if="item.type === 'action'" class="flex items-center gap-1">
                    <button
                      ...
                      :key="actionIndex" @click="$emit(action.event, row)" :title="action.name"
      ...
      <div v-if="pagination && filteredData.length > pageSize"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
          <div class="text-sm text-gray-700 dark:text-gray-300 text-center sm:text-left">
            {{ $t('showing') }} {{ ((currentPage - 1) * pageSize) + 1 }} {{ $t('to') }} {{ Math.min(currentPage *
              pageSize,
            filteredData.length) }}
