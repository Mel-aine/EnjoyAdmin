        ...
        <div class="mt-3">
                ...
                <form v-else @submit.prevent="handleSubmit">
                    ...
                    <div v-if="formData.noShowType === 'individual' && reservationRooms.length > 0" class="mb-4">
                        ...
                        <div class="space-y-2 max-h-40 overflow-y-auto">
                            <label v-for="room in reservationRooms" :key="room.id"
                                class="flex items-center p-2 border rounded" :class="{
                                    'hover:bg-gray-50 cursor-pointer dark:hover:bg-gray-700': room.status !== 'no_show',
                                    'bg-gray-100 dark:bg-gray-800 cursor-not-allowed opacity-60': room.status === 'no_show'
                                }">
                                ...
                                <div class="ml-2 flex-1">
                                    <span class="text-sm"
                                        :class="room.status === 'no_show' ? 'text-gray-500 dark:text-gray-400' : 'text-gray-700 dark:text-gray-300'">
                                        {{ room.room?.roomNumber }} - {{ room.guest?.displayName || room.guestName
                                        }}
                                    </span>
                                    <span v-if="room.status === 'no_show'"
                                        class="ml-2 text-xs text-red-600 dark:text-red-400 font-medium">
                        ...
                        <p v-if="allRoomsNoShow" class="mt-2 text-sm text-red-600 dark:text-red-400">
