  ...
  <div ref="selectWrapper" class="w-full">
    ...
    <div :class="['relative font-sans', (disabled || isLoading) ? 'cursor-not-allowed opacity-50' : 'cursor-pointer']">
      ...
      <ul v-if="isDropdownOpen && !isLoading"
        class="custom-scrollbar absolute top-full left-0 right-0 z-999 mt-1 rounded-b-lg max-h-60 overflow-y-auto text-lg sm:text-base bg-white border-2 border-t-0 border-purple-100 shadow-lg dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100"
        ...
        <li v-for="folio in folioOptions" :key="folio.id" @click="selectFolio(folio)" :class="[
          ...
          selectedFolio?.id === folio.id ? 'bg-brand-50 text-brand-700 dark:bg-blue-900 dark:text-blue-200' : ''
