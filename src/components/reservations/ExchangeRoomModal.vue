    ...
    <div class="space-y-6">
      ...
      <div v-else-if="reservation">
        ...
        <div v-for="(res, ind) in reservation.reservationRooms" :key="ind">
          ...
          <!-- Guest Card -->
          <div
            class="border border-blue-200 rounded-lg p-4 mb-6 cursor-pointer"
            :class="selectedIndex === ind ? 'ring-2 ring-green-500' : ''"
            @click="selectReservationRoom(ind)"
          >
            <div class="flex items-center justify-between">
              <div class="flex  gap-3 justify-start align-top items-start">
                ...
                <div class="text-md text-gray-500 flex flex-col justify-between h-full">
                  ...
                  <div class="flex gap-7">
                    ...
                    <div class="">
                      <div class="text-xs text-gray-500 mb-1 mt-8 ">Room</div>
                      <div class="text-sm font-medium">{{ res.room?.roomNumber }}</div>
                    </div>
                  </div>
                </div>

              </div>
              <div v-if="selectedIndex === ind" class="flex items-center">
...
const isAssignButtonEnabled = computed(() => selectedIndex.value !== null)

const selectReservationRoom = (index: number) => {
...
const confirmRoomSelection = async () => {
  ...
  if (roomSelections.length > 0) {
    ...
    try {
      await postRoomMoveReservation(props.reservationId, { reservationRooms: roomSelections })
