    ...
    <template #header>
      ...
      <div class="p-4 space-y-4" v-if="!isLoading">
        <div class="text-sm text-gray-600">
          {{ t('Select checked-in rooms to undo.') }}
        </div>

        <div class="space-y-3">
          <div v-for="room in checkedInRooms" :key="room.id" class="flex items-center justify-between border rounded p-3">
            <div>
        ...
        <div v-if="checkedInRooms.length === 0" class="text-sm text-gray-500">{{ t('No checked-in rooms to undo.') }}</div>

        <div class="space-y-2">
          <label class="text-sm font-medium">{{ t('notes') }}</label>
          <textarea v-model="formData.notes" class="w-full border rounded p-2" rows="3" :placeholder="t('Reason for undo')"></textarea>
        </div>
      </div>
...
const { t } = useI18n()
const { performUndoCheckIn, isUndoingCheckIn } = useReservation()
...
const checkedInRooms = computed(() =>
  reservationRooms.value.filter((r: any) => r.status === 'checked_in' || r.checkedIn || !!r.actualCheckInTime)
)
...
const fetchDetails = async () => {
  ...
  try {
    ...
    // pré-sélectionner toutes les chambres en check-in
    formData.value.selectedRoomIds = checkedInRooms.value.map((r: any) => r.id)
  } finally {
...
const performUndo = async () => {
  ...
  const response = await performUndoCheckIn(props.reservationId, payload)
  emit('success', {
