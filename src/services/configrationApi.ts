...
export const assignExistingGuestToRoom = async (
  reservationRoomId: number,
  guestId: number,
  replaceMode: boolean = true
): Promise<any> => {
  try {
    const response = await axios.put(
      `${URL}/reservation-rooms/${reservationRoomId}/assign-existing-guest`,
      {
        guestId,
        replaceMode
      },
 ...
 * get availiabele room by typeId
 */
export const getFrontofficeBookingDataId = (
  startDate?: string,
  endDate?: string
): Promise<AxiosResponse<any>> => {
  const params = {
  ...
  return axios.get(
    `${API_URL()}/rooms/frontoffice/bookingrooom`,
    {
      ...getHeaders(),
      params
    }
